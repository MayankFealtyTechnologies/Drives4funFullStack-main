...
import { authorizeMe, getQuotations, updateNewQuotation } from "./service";
...
const List = () => {
    ...
    const getAllQuatation = async () => {
        var res = await getQuotations({ status: selected, page: page + 1 })
            ...
            renderCell: (params) => {
                return params?.row?.status === "pending" ?
                    <div style={{ display: "flex", justifyContent: 'space-around' }
                        ...
                        <div onClick={async () => {
                            await updateNewQuotation({ _id: params?.row?._id, status: "accepted" })
                            getAllQuatation({})
                        }}>
                            <CheckCircleOutlineIcon style={{ color: "green", fontSize: 24 }} />
                        </div>
                        <div onClick={async () => {
                            await updateNewQuotation({ _id: params?.row?._id, status: "rejected" })
                            getAllQuatation({})
                        }}>
                            <CancelIcon style={{ color: "red", fontSize: 24, marginLeft: 20 }} />
                        </div>
                    </div > :
